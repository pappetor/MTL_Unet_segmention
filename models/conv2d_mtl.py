...
class _ConvNdMtl(Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride,
        ...
        self.groups = groups
        if transposed:
            #weight和bias的参数不能调整，而mtl_weight和mtl_bias的参数可以调整
            #torch.Tensor（设置尺寸大小）
            #weight的尺寸格式为：Co、Ci、H、W
    ...
    def reset_parameters(self):
        """将weight、mtl_weight和bias、mtl_bias初始化"""
        n = self.in_channels
...
class Conv2dMtl(_ConvNdMtl):
    ...
    def forward(self, inp):
        #expand：将mtl_weight扩展为self.weight的维度
        new_mtl_weight = self.mtl_weight.expand(self.weight.shape)
        #mul函数，对应位置相乘后相加，不是矩阵乘法
