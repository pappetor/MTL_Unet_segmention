...
class BaseLearner(nn.Module):
    ...
    输入为Unet预测后的像素图，输出为同尺寸的像素图，但是通道变为了支持集中的way
    """
    def __init__(self, args):
        super().__init__()
        self.args = args
        self.vars = nn.ParameterList()
        #weight:就是kernel 尺寸：Co Ci H W
    ...
    def forward(self, input_x, the_vars=None):
        if the_vars is None:  #the_vars:fast_weight
            the_vars = self.vars
        norm1=self.norm1
        norm2=self.norm2
        wt = the_vars[0]
        bias = the_vars[1]
        output=F.conv2d(norm2(norm1(input_x)),wt,bias,stride=1,padding=1)
        return output
